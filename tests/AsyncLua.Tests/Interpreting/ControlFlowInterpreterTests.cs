...
public class ControlFlowInterpreterTests
{
	...
	public void GenericFor_SimpleIterator_SumsValues()
	{
		...
		//   R6 = sum accumulator
		//
		// TFORCALL 0, 1: calls R0(R1,R2), result at R3, updates R1,R2
		// TFORLOOP 0, sBx: if R1 != nil → jump to body
		...
		var proto = MakeProto(new[]
		{
            ...
            new Instruction(OpCode.TFORCALL, a: 0, b: 0, c: 1, flags: OpFlags.None),  // R3 = f(R1,R2); R1,R2 updated
            ...
            // Check (pc=8)
            // TFORLOOP uses A=0 (same base as TFORCALL) to check R1 (state) and R2 (var)
            new Instruction(OpCode.TFORLOOP, a: 0, b: unchecked((ushort)-2), c: 0, flags: OpFlags.SignedBX), // if R1!=nil: goto body
